  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        {categories.length === 0 ? (
          <p className="text-xs text-gray-500 dark:text-gray-400">No collections yet. Add one from Quick Actions.</p>        ) : (
          categories.map((c) => {
            ...
            return (
              <div key={c.id} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50 dark:bg-gray-700">
                ...
                <div className="text-sm">
                  ...
                  <div className="text-xs text-gray-500 dark:text-gray-400">{count} prompt{count === 1 ? '' : 's'}</div>
                </div>
              </div>
            )
          })
