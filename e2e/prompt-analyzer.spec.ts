...
test.describe("Prompt Analyzer", () => {
  ...
  test("analyzes a prompt end to end and saves the optimized version to the library", async ({ page }) => {
    ...
    // NOTE: I haven't seen AIWorkspace.tsx's source — this assumes the
    // "Analyze Prompt" tab is a clickable element with that exact text,
    // based on screenshots only. If this fails, it's the first place to check.
    ...
    // ASSUMPTION: submit button is the LAST "Analyze Prompt" element in the
    // DOM (the tab nav renders first). Adjust if this turns out wrong.
